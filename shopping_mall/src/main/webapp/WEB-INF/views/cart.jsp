...
<link rel="stylesheet" href="/resources/css/cart.css?after">
<script
	...
	<div class="wrap">
		...
		<div class="content_subject"><span>장바구니</span></div>
						...
						<c:forEach items="${cartInfo}" var="ci">
							<tr>
								...
								<input type="checkbox" class="individual_cart_checkbox input_size_20" checked="checked">
									<input type="hidden" class="individual_itemPrice_input" value="${ci.itemPrice}">
									<input type="hidden" class="individual_salePrice_input" value="${ci.salePrice}">
									<input type="hidden" class="individual_itemCount_input" value="${ci.itemCount}">
								...
								<td class="td_width_4 table_text_align_center">
									<div class="table_text_align_center quantity_div">
										<input type="text" value="${ci.itemCount}" class="quantity_input">	
								...
								<td class="td_width_4 table_text_align_center">
								<button class="delete_btn" data-carid="${ci.cartId}">삭제</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<form action="/cart/update" method="post" class="quantity_update_form">
				...
				<input type="hidden" name="memberId" value="${member.memberId}">  <!--세션 유저아이디  -->
			</form>
...
/* 체크여부에따른 종합 정보 변화 */
$(".individual_cart_checkbox").on("change", function(){ //체크박스 변환에 따라 동작
	...
	$(".all_check_input").on("click", function(){
		...
		if($(".all_check_input").prop("checked")){
			// prop() 메서드는 인자가 하나일 경우 호출한 객체가 인자로 부여한 속성의 값 프로퍼티로 가져옴
    ...
    e.preventDefault();
    const cartId = $(this).data("carid");
    console.log("카트번호=" + cartId);
